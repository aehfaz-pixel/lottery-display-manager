...
**Follow-up, now confirmed resolved (2026-08-18):** a report of one import "faltering" then a retry working quickly came in after this fix shipped. Reviewed the debug log from the real installed app: on the actual v1.0.37 build, two consecutive real imports both completed cleanly and fully automatically in ~12-20 seconds each, with the new logging showing every step (`stageAPI called` → IndexedDB restore, 77 entries, ~100ms → `RESTART received` → `flushStorageData()` → `app.relaunch()` → new `APP LAUNCH` → `applied 20 of 20 keys` → `CONFIRMED applied`) — no gaps, no manual intervention needed. The "faltered" report was almost certainly from an install still on an older version (before the `main.js` `get-app-version-sync` handler was restored — see the blank-window incident earlier the same day), not a problem with the fixed v1.0.37 code. **Considered closed** — no further action unless it recurs on a confirmed-current install.
